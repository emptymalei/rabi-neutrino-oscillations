...
\begin{itemize}
    \item Font family of the plot? It looks weird.
\end{itemize}
}
}
...
\begin{figure}[!htbp]
                ...
                \caption{Single frequency matter profile and Rabi oscillation.  Black cross is the transition probability between two background mass eigenstates for the neutrinos with matter perturbation $A_1\sin(k_1x)$. During the calculation, $\lambda_0$ is set to $0.5$ of the MSW resonance requirement. The red line is the probabilities predicted using Rabi formula.}
...
Another well researched matter profile is the periodic castle wall matter profile. Within one period the profile,
\begin{equation}
...
To calculate the transitions between two mass states of background matter profile $\Lambda_0$, we rotate to the background matter basis with respect to $\Lambda_0$, in which the transition is zero when variations of matter profile vanishes. The Hamiltonian
\begin{widetext}
\begin{equation}
H = - \frac{1}{2}\omega_m \sigma_3  + \frac{1}{2} \sum_{q=1}^{\infty} \Lambda_q \cos\left( (2q-1)k_0  x \right) \cos 2\theta_m \sigma_3 - \frac{1}{2} \sum_{q=1}^{\infty} \Lambda_q \cos\left( (2q-1)k_0 x \right) \sin 2\theta_m \sigma_1,
\end{equation}
\end{widetext}
determines the transitions between the two background matter states.

Similar to single frequency matter profile, the varying $\sigma_3$ terms have limited effects on the transition probabilities when the system is close to resonance, since the rotating frequency of Hamiltonian vector associated with $\sigma_1$ can always match the background energy gap $\omega_{\mathrm{m}}$,  which is shown exactly later. We expect the .
