...
To determine the important modes, the resonance width for each mode which is determined by $B_n$ for single frequency matter profile or $B_N$ for multi-frequency matter profile, the deviation from exact resonance (i.e., detuning) which is calculated as $\lvert n k_1 - \omega_{\mathrm{m}} \rvert$ for single frequency matter profile or $\lvert \sum_a n_a k_a - \omega_{\mathrm{m}} \rvert$ for multi-frequency matter profile, and oscillation wavelength of each mode compared to the size of the physical system should be considered. By utilizing the theory of Rabi oscillation, we know that the oscillation wavelength of each mode is determined by the Rabi frequency.

More specifically, we define the relative detuning $Q = \lvert B_n/( n k_1 - \omega_{\mathrm{m}}  )\rvert$, which measures how well the resonance is. For exact resonance, $Q=0$, while large $Q$ says the system is too far away from the resonance of this specific mode.
